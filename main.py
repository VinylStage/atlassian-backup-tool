...
def interactive_backup_flow():
    ...
    try:
        ...
        logger.info(
            "이제부터 '%s' 파일을 사용하여 다음 작업을 코딩하시면 됩니다.", output_path
        )
        logger.info("예시: 다운로드된 JSON 파일을 HTML로 변환하기")
        logger.info("1. parser.py가 이 파일을 읽도록 INPUT_PATH를 수정하거나,")
        logger.info(
            "2. 아래처럼 커맨드 라인 인자로 파일 경로를 넘겨주도록 parser.py를 수정할 수 있습니다."
        )
        logger.info("   (예: python parser.py %s)", output_path)
        logger.info("==========================================================")
